...
if __name__ == "__main__":
    ...
    query_1_base = hourly_lines \
        .map(lambda x: ((get_country(x), get_year(x)), (get_co2_intensity(x), get_c02_free(x), 1))) \
        .cache()
    ...
    query_1 = avg_by_country \
        .join(min_by_country) \
        .join(max_by_country) \
        .map(lambda x: (x[0], x[1][0][0][0], x[1][0][0][1], x[1][0][1][0], x[1][0][1][1], x[1][1][0], x[1][1][1]))\
        .sortBy(lambda x: x[0], True)

    print(tabulate(query_1.collect(),
                   headers=["Country, Year",
                            "Avg CO2 Intensity", "Avg C02 Free",
                            "Min CO2 Intensity", "Min C02 Free",
                            "Max CO2 Intensity", "Max C02 Free"],
                   tablefmt="psql"))
